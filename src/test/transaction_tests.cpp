	    ...
	    for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                BOOST_CHECK_MESSAGE(VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout],
                                                 witness, verify_flags, TransactionSignatureChecker(&tx, i, amount, TOKENID_ZERO, 0, txdata), &err),
                                    strTest);
...
void CheckWithFlag(const CTransactionRef& output, const CMutableTransaction& input, int flags, bool success)
{
    ...
    bool ret = VerifyScript(inputi.vin[0].scriptSig, output->vout[0].scriptPubKey, &inputi.vin[0].scriptWitness, flags, TransactionSignatureChecker(&inputi, 0, output->vout[0].nValue, output->vout[0].tokenID, output->vout[0].nTokenValue), &error);
    assert(ret == success);
}
...
BOOST_AUTO_TEST_CASE(test_witness)
{
    ...
    UpdateTransaction(input1, 0, CombineSignatures(output1->vout[0].scriptPubKey, MutableTransactionSignatureChecker(&input1, 0, output1->vout[0].nValue, output1->vout[0].tokenID, output1->vout[0].nTokenValue), DataFromTransaction(input1, 0), DataFromTransaction(input2, 0)));
    CheckWithFlag(output1, input1, STANDARD_SCRIPT_VERIFY_FLAGS, true);
