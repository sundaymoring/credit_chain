...
std::pair<CTokenId, CAmount> CWallet::GetTokenIdAndDebit(const CTxIn &txin, const isminefilter& filter) const
{
    {
        LOCK(cs_wallet);
        map<uint256, CWalletTx>::const_iterator mi = mapWallet.find(txin.prevout.hash);
        if (mi != mapWallet.end())
        {
            ...
            if (txin.prevout.n < prev.tx->vout.size())
                if (IsMine(prev.tx->vout[txin.prevout.n]) & filter)
                    return std::make_pair(prev.tx->vout[txin.prevout.n].tokenId, prev.tx->vout[txin.prevout.n].nValue);
        }
    }
...
std::pair<CTokenId, CAmount> CWallet::GetTokenIdAndCredit(const CTxOut& txout, const isminefilter& filter) const
{
    if (!TokenRange(txout.nValue))
        throw std::runtime_error(std::string(__func__) + ": value out of range");
    if (IsMine(txout) & filter) {
        return std::make_pair(txout.tokenId, txout.nValue);
    }
    return std::make_pair(TOKENID_ZERO, 0);
}
...
CAmount CWallet::GetTokenDebit(const CTokenId& tokenid, const CTransaction& tx, const isminefilter& filter) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        std::pair<CTokenId, CAmount> token = GetTokenIdAndDebit(txin, filter);
        if (token.first == tokenid)
            nDebit += token.second;
        if (!TokenRange(nDebit))
            throw std::runtime_error(std::string(__func__) + ": value out of range");
    }
    return nDebit;
}
...
CAmount CWallet::GetTokenCredit(const CTokenId& tokenid, const CTransaction& tx, const isminefilter& filter) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        std::pair<CTokenId, CAmount> token = GetTokenIdAndCredit(txout, filter);
        if (tokenid == token.first) {
            nCredit += token.second;
            if (!TokenRange(nCredit))
                throw std::runtime_error(std::string(__func__) + ": value out of range");
        }
    }
    return nCredit;
                           ...
                           list<COutputEntry>& listSent, CAmount& nFee, string& strSentAccount, const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < tx->vout.size(); ++i)
    {
        ...
        COutputEntry output = {address, txout.nValue, txout.tokenId, (int)i};
...
/*        if (fDebitCached)
        ...
        debit +=  pwallet->GetTokenDebit(tokenid, *this, ISMINE_SPENDABLE);
    }
    if(filter & ISMINE_WATCH_ONLY)
    {
/*        if(fWatchDebitCached)
        ...
        debit +=  pwallet->GetTokenDebit(tokenid, *this, ISMINE_WATCH_ONLY);
    }
    return debit;
}
...
/*        if (fCreditCached)
        ...
        credit += pwallet->GetTokenCredit(tokenid, *this, ISMINE_SPENDABLE);
    }
    if (filter & ISMINE_WATCH_ONLY)
    {
/*        if (fWatchCreditCached)
        ...
        credit += pwallet->GetTokenCredit(tokenid, *this, ISMINE_WATCH_ONLY);
    }
    return credit;
}
...
//            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            std::pair<CTokenId, CAmount> ic = pwallet->GetTokenIdAndCredit(txout, ISMINE_SPENDABLE);
            if (ic.first == tokenId){
                nCredit += ic.second;
            }
            if (!MoneyRange(nCredit))
...
CAmount CWalletTx::GetAvailableTokenCredit(const CTokenId& tokenid, bool fUseCache) const
{
    ...
    // coinbase will never have token
    if ((IsCoinBase() || IsCoinStake()))
        return 0;
...
//        return nAvailableCreditCached;
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++)
    {
        ...
        {
            const CTxOut &txout = tx->vout[i];
            std::pair<CTokenId, CAmount> token = pwallet->GetTokenIdAndCredit(txout, ISMINE_SPENDABLE);
            if (tokenid == token.first)
                nCredit += token.second;
            if (!TokenRange(nCredit))
                throw std::runtime_error("CWalletTx::GetAvailableTokenCredit() : value out of range");
        }
    }
...
CAmount CWallet::GetBalance(const CTokenId& tokenId) const
{
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted())
                nTotal += pcoin->GetAvailableCredit(tokenId);
        }
    }
...
CAmount CWallet::GetTokenBalance(const CTokenId& tokenid) const
{
    CAmount nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            const CWalletTx* pcoin = &(*it).second;
            if (pcoin->IsTrusted())
                nTotal += pcoin->GetAvailableTokenCredit(tokenid);
        }
    }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO && ( isall || tokenid == pcoin->tx->vout[i].tokenId ) &&
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, bool overrideEstimatedFeeRate, const CFeeRate& specificFeeRate, int& nChangePosInOut, std::string& strFailReason, bool includeWatching, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, bool keepReserveKey, const CTxDestination& destChange)
{
    ...
    for (size_t idx = 0; idx < tx.vout.size(); idx++)
    {
        ...
        CRecipient recipient = {txOut.scriptPubKey, txOut.nValue, txOut.tokenId, setSubtractFeeFromOutputs.count(idx) == 1};
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, tokencode code)
{
    ...
    if (code == TTC_SEND) {
        ...
        if (scriptInfo.burnAmount != vecSend[1].nAmount) {
...
//            std::vector<COutput> vAvailableToken;
            // ethier token or cnet, only different is tokenID.
...
//            }
            ...
            while (true)
            {
                ...
                CAmount nValueToSelect = (code == TTC_ISSUE ? 0 : nValue);
                ...
                for (const auto& recipient : vecSend)
                {
                    CTxOut txout(recipient.nAmount, recipient.scriptPubKey, recipient.tokenId);
                ...
                if (nChange > 0)
                {
                    ...
                    CTxOut newTxOut(nChange, scriptChange, tokenId);
